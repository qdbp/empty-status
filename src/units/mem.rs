...
impl Unit for Mem {
    ...
    async fn read(&mut self) -> Result<HashMap<String, Value>> {
        ...
        result.insert("used_KiB".to_string(), json!(used_kib));
        result.insert("used_frac".to_string(), json!(used_frac));
    ...
    fn format(&self, data: &HashMap<String, Value>) -> String {
        ...
        let used_kib = data.get("used_KiB").and_then(|v| v.as_u64()).unwrap_or(0);
        ...
        format!("mem [used {} GiB ({}%)]", formatted_gib, formatted_percent)
    }
}
